  ...
  return (
    ...
    >
      <div
        ...
        className={`w-full h-full flex-1 overflow-hidden relative ${showOnHover ? 'hover:pr-3 sm:hover:pr-4 md:hover:pr-2' : 'pr-2'} transition-all duration-300 ease-out`}
      ...
      <div
        ref={scrollbarRef}
        className={`absolute right-1 sm:right-2 md:right-1 top-0 h-full w-1.5 sm:w-2 md:w-1.5 bg-transparent/50 backdrop-blur-sm rounded-full transition-all duration-300 group ${trackColor} ${(showOnHover || isScrolling || isDragging) ? 'opacity-100 scale-x-100' : 'opacity-0 scale-x-0'}`}
      >
        <div
          ref={thumbRef}
          className={`absolute left-0 w-full rounded-full transition-all duration-200 ease-out shadow-lg cursor-grab active:cursor-grabbing select-none touch-none ${thumbColor}`}
          style={{ height: `${thumbHeight}px`, top: 0, transform: `translateY(${thumbTop}px)` }}
          onMouseDown={(e) => { e.preventDefault(); handleStartDrag(getClientY(e)); }}
          onTouchStart={(e) => { e.preventDefault(); handleStartDrag(getClientY(e)); }}
        />
        {isScrolling && (
